...
app.controller('homeTasksController', function ( $scope, $http, Scopes, $location, $timeout) {
    ...
    var loadAllActiveTasks = $http({
        ...
        .error(function(data, status, headers, config) {
            alert("Loading Tasks Request failed");
    });
    ...
    var loadRecentlyCompletedTasks = $http({
        ...
        .error(function(data, status, headers, config) {
            alert("Recently Completed Tasks Request failed");
    });
    ...
    $scope.updateOverdueBadge = function(){
        // Add a badge if there isn't one and there are overdue tasks.
        if($("#overdue_badge").length == 0 && $scope.overdue_tasks.length > 0){
            var badge = '<span class="w3-badge w3-margin-left w3-red" id="overdue_badge">'+$scope.overdue_tasks.length+'</span>';
            $("#home_page_navbar").append(badge);

        // Update badge if the overdue tasks # changed. First remove badge & add new one.
        }else if($scope.overdue_tasks.length > 0){
            $("#overdue_badge").remove();
            var badge = '<span class="w3-badge w3-margin-left w3-red" id="overdue_badge">'+$scope.overdue_tasks.length+'</span>';
            $("#home_page_navbar").append(badge);
        // Completely remove badge if there are no overdue tasks.
        }else{
            if($("#overdue_badge").length == 0){
                $("#overdue_badge").remove();
            }
        }
    }
